...
- 最新上游差距查询（仅 fetch，未合并）：`release/2.1.10@8f8e469` 相对已获取的
  `upstream/main@4d66b7b638c9cb9d533ffe95f23a56b822af90e2` 领先 152、落后 8 个提交。
  查询前工作区干净；本次仅更新此状态记录，未修改源码、构建或执行真机验证。
  已核对这 8 个提交的改动：下载分P排序、直播关键词/用户屏蔽、消息未读角标、Linux WebView、
  直播屏蔽列表传参与网页路由整理、权限依赖升级、上游 2.1.3 版本声明、缺失视频画质补充容错。
  已用 `git merge-tree --write-tree --name-only HEAD upstream/main` 模拟合并，唯一文本冲突为
  `pubspec.yaml` 的版本行：本地 `2.1.10+2026082502` 与上游 `2.1.3+1`；正式同步应保留本地版本。
  直播控制器、视频控制器和网页路由工具可自动合并，但仍待语义审查、测试和真机回归。
  模拟未修改分支、索引或源码工作区；当前仅此状态文档未提交，正式合并前需先保存该修改。
